...
const TeamPage = ({ history, getTeamVideos, teamVideos }) => {
    ...
    <>
      ...
      >
        ...
        >
          {teamVideos &&
            teamVideos.upcoming &&
            teamVideos.upcoming.map((stream, index) => {
              if (index < 3) {
                let thumbnail = null;
                if (stream.thumb1) {
          ...
          <Container fluid>
            <Row>
              <Col sm="12" className="overflow-hidden">
                ...
                {teamVideos && teamVideos.live && teamVideos.live.length > 0 ? (
                  <div id="favorites-contens">
                    ...
                    >
                      {teamVideos.live.map((stream, index) => {
                        let thumbnail = null;
                        if (stream.thumb1) {
                          thumbnail = thumbUrl + stream.thumb1;
                        } else if (stream.thumb2) {
                ...
                teamVideos.archive.length > 0 ? (
                  <div id="upcoming-contens">
                    ...
                    <Swiper
                      ...
                      className="favorites-slider list-inline row p-0 m-0 iq-rtl-direction"
                    >
                      {teamVideos.archive.map((stream, index) => {
                        return (
                          <SwiperSlide className="slide-item" key={index}>
...
const mapStateToProps = (state) => ({
  teamVideos: state.videos.team_videos,
});

const mapDispatchToProps = (dispatch) => {
  return {
    getTeamVideos: (publish_point) => {
      dispatch(videoActions.getTeamVideos(publish_point));
    },
  };
};
export default compose(connect(mapStateToProps, mapDispatchToProps))(TeamPage);
